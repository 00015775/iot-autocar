...
The **HW-504 joystick** is physically wired to an Arduino. The Arduino reads the joystick's **SW**, **X** and **Y** pin values and sends these coordinates/values to the computer through a USB connection. Since the Raspberry Pi cannot be connected to the Arduino in this setup (*[it can be connected](https://roboticsbackend.com/raspberry-pi-arduino-serial-communication/#:~:text=You%20can%20get%20help%20from%20the%20Arduino%20Serial%20reference%20and%20the%20pySerial%20API%20reference.) but in this project the intention is to remotely control the car*), the computer acts as a bridge as it receives joystick inputs from Arduino and then sends that data across the local network to the RPi. RPi is connected to the same subnet as the computer, hence RPi can receive these coordinate values and interpret them as control commands for the car to be remotely controlled. 
